...
<script>
  ...
  import firebase from 'firebase'
    ...
    mounted(){
      ...
      getTargetTime(){
        let controls = this.controls
        db.collection("controlsTargetTime").doc("control").onSnapshot(function(doc) {
            let con = doc.data();
            controls.year = con.year
            controls.month =con.month
            controls.day = con.day
            controls.hour = con.hour
            controls.min = con.min
            controls.sec = con.sec
        })
      },
			initCanvas() {
        if(! /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ){
          ...
          this.wh = this.canvas.height = window.screen.availHeight;
          this.ww = this.canvas.width = window.screen.availWidth;
        } 
      },
      init() {
        let time = this.time
      ...
      draw() {
        ...
        ctx.save()
          ...
          this.circles.forEach((obj,id)=>{
            obj.draw(ctx,this.degToPI,time)
          })
          ctx.fillStyle="white"
          ctx.font = "20pt Arial";
      ...
      setDatGui(){
          ...
          gui.domElement.style = 'position:relative;top:50px;left:0px;width:50px';
      }*/
      }
	}
</script>
